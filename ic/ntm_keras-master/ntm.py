...
class NTM(Recurrent):
    ...
    def build(self, input_shape):
        ...
        #controller_input = Input(shape=(None, self.controller_input_dim))
        if(self.type_controller == 'lstm'):
            self.controller = LSTM(units=self.controller_output_dim, 
                                   ...
                                   name = "controller")
            self.controller.build(input_shape=(None, input_length, input_dim))
            print("Controllador LSTM criado!!")
        elif(self.type_controller == 'dense'):
            self.controller = Dense(
                name = "controller",
                activation = 'linear',
                bias_initializer = 'zeros',
                units = self.controller_output_dim,
                input_shape = (input_length, self.controller_input_dim))
            self.controller.build(input_shape=(None, input_length, input_dim))
            print("Controllador Dense criado!!")
        else:
            print("Tipo inválido de controllador!!")
            exit()
        ...
        if (self.type_controller == 'lstm'):
            ...
            self.state_spec = [InputSpec(shape=(self.output_dim,)),                          # old_ntm_output
                               InputSpec(shape=(self.n_slots, self.m_depth)),                # Memory
                               InputSpec(shape=(self.read_heads, self.n_slots)),             # weights_read
                               InputSpec(shape=(self.write_heads, self.n_slots)),            # weights_write
                               InputSpec(shape=(self.controller_output_dim,)),               # init_h
                               InputSpec(shape=(self.controller_output_dim,))]               # init_c
        else:
        ...
        super(NTM, self).build(input_shape)
    ...
    def get_initial_state(self, X):
        ...
        init_old_ntm_output = K.ones((self.output_dim,), name="init_old_ntm_output")*0.42 
        init_M = K.ones((self.n_slots , self.m_depth), name='main_memory')*0.042
        init_wr = np.zeros((self.read_heads, self.n_slots))
        init_wr[:,0] = 1
        init_wr = K.variable(init_wr, name="init_weights_read")
        init_ww = np.zeros((self.write_heads, self.n_slots))
        init_ww[:,0] = 1
        init_ww = K.variable(init_ww, name="init_weights_write")
        init_h = K.zeros((self.controller_output_dim,))
        init_c = K.zeros((self.controller_output_dim,))
        if (self.type_controller == 'lstm'):
...
#        M = tf.Print(M, [beta, g, s, gamma], message='get weights beg2: ')
    ...
    def _run_controller(self, inputs, read_vector, state):
        controller_input = K.concatenate([inputs, read_vector], axis=0)
        print("Controller_shapee ", controller_input.shape)
    ...
    def _split_and_apply_activations(self, controller_output):
        ...
        controller_instructions_read = [
                tf.split(single_head_data, np.asarray([self.m_depth, 1, 1, self.shift_range, 1]), axis=1) for 
                single_head_data in controller_instructions_read]
        
        controller_instructions_write = [
                tf.split(single_head_data, np.asarray([self.m_depth, 1, 1, self.shift_range, 1, self.m_depth, self.m_depth]), axis=1) for 
    ...
    def step(self, layer_input, states):
        ...
        weights_read_tm1 = K.reshape(weights_read_tm1,
                (self.read_heads, self.n_slots))
        weights_write_tm1 = K.reshape(weights_write_tm1, 
                (self.write_heads, self.n_slots))
        ...
        memory_read_input = K.concatenate([self._read_from_memory(weights_read_tm1[i], M) for i in range(self.read_heads)])
        memory_read_input = K.reshape(memory_read_input, [self.read_heads, self.n_slots])
        print(memory_read_input)
        
        #print(len(inp))
        
        #memory_read_input = K.reshape(memory_read_input, shape=(None, memory_read_input.shape[0]))
        ...
        controller_output = self._run_controller(layer_input, memory_read_input, None)
